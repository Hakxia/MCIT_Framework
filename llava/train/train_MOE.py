...
import torch
import transformers   # CoIN25 修改了transformers做了EMA的适配，MoELoRA是没有EMA的，所以不要import修改的transformers ！！！
...
def find_all_linear_names(model):
    ...
    # lm_head 是模型输出阶段的关键组件，通常是一个 线性层（Linear Layer），作用是将隐藏状态映射到词表（vocabulary）大小的向量，用于预测下一个 token 的概率分布。
    # LoRA 通常不会直接作用于 lm_head，而是针对注意力层的 q_proj、v_proj 等矩阵进行低秩适配。
...
def load_model_from_previous_task(model, previous_task_model_path):
    ...
    model.base_model.model.load_state_dict(non_lora_trainables, strict=False)  # 坑
...
def train():
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            # 本质上llava还是借用的llama，这里需要加载llama的预训练权重，至于视觉侧，会在进入llama前将视觉-语言的token拼接好
    ...
    # 在 训练阶段（尤其是使用梯度检查点时）：
    # 不需要 KV Cache（因为是 teacher forcing，一次性输入整个序列）
    # 开启 use_cache 反而会增加显存占用（存储所有层的 past_key_values）
    ...
    trainer.save_state()
    # 训练完要设回 True
    # 因为：保存的模型通常用于推理，推理时我们希望默认启用 KV Cache 来加速 generate()
