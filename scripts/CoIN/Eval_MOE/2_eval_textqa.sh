...
if [ ! -n "$2" ] ;then
    MODELPATH='./Checkpoints/LLaVA/CoIN/MoE_LoRA/TextVQA/llava-1.5-7b-moe_lora'
else
    MODELPATH=$2
fi
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.CoIN.model_text_vqa \
        ...
        --question-file ../../dataset/CoIN/Instructions_Refined/Instructions_Origin/TextVQA/val.json \
...
python -m llava.eval.CoIN.eval_textvqa \
    --annotation-file ../../dataset/CoIN/CL_dataset/TextVQA/TextVQA_0.5.1_val.json \
