...
if [ ! -n "$2" ] ;then
    MODELPATH='./Checkpoints/LLaVA/CoIN/Grounding_llava-1.5-7b-moe_lora'
else
    MODELPATH=$2
fi
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.CoIN.model_vqa_cc_instruction \
        ...
        --question-file ../../dataset/CoIN/Instructions_Refined/Instructions_Origin/Grounding/test.json \
...
python -m llava.eval.CoIN.eval_grounding \
    --test-file ../../dataset/CoIN/Instructions_Refined/Instructions_Origin/Grounding/test.json \
