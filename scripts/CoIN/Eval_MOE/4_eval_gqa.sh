...
if [ ! -n "$2" ] ;then
    MODELPATH='./Checkpoints/LLaVA/CoIN/GQA_llava-1.5-7b-moe_lora'
else
    MODELPATH=$2
fi
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.CoIN.model_gqa \
        ...
        --question-file ../../dataset/CoIN/Instructions_Refined/Instructions_Origin/GQA/test.json \
...
python -m llava.eval.CoIN.eval_gqa \
    ...
    --question-dir ../../dataset/CoIN/CL_dataset/GQA \
