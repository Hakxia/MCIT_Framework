...
if [ ! -n "$2" ] ;then
    MODELPATH='./Checkpoints/LLaVA/CoIN/ImageNet_llava-1.5-7b-moe_lora'
else
    MODELPATH=$2
fi
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.CoIN.model_vqa_cc_instruction \
        ...
        --question-file ../../dataset/CoIN/Instructions_Refined/Instructions_Origin/ImageNet/test.json \
...
python -m llava.eval.CoIN.eval_ImagetNet \
    --test-file ../../dataset/CoIN/Instructions_Refined/Instructions_Origin/ImageNet/test.json \
